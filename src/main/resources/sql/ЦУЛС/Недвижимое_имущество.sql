...
from pfr_dashboard.culs_uplata where  toFloat32(replace((case when amount_to_be_calculated='' then '0' else amount_to_be_calculated end),',','.'))>0 and KBK = 104101 or KBK = 104102 and IIN_BIN is not null and IIN_BIN!=''
) AS subquery
WHERE iin_bin = '$P-IIN'
